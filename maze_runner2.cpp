...
bool walk(Position pos) {
    ...
    for (const auto& next_pos : directions) {
        if (is_valid_position(next_pos.row, next_pos.col)) {
            if (!found_exit) {
                // Continuar com a thread atual para explorar o primeiro caminho
                if (walk(next_pos)) {
                    found_exit = true;
                }
            } else {
                // Criar uma nova thread para explorar caminhos adicionais
                threads.emplace_back([next_pos, &found_exit]() {
                    if (walk(next_pos)) {
                        found_exit = true;
                    }
                });
    ...
    // Esperar todas as threads adicionais terminarem
